...
export async function POST(request: Request) {
   ...
   db_res = await db.insert(TaskTable).values({
      title,
      description,
...
export async function DELETE(request: Request){
   ...
   try {
      const db_res = await db.delete(TaskTable)
      .where(eq(TaskTable.id, taskId)).returning()

      if (!db_res) {
         return new Response('Task not found or you do not have permission to delete this task', { status: 404 })
      }

      revalidatePath('/tasks')
      const response = new Response(JSON.stringify({ message: 'Task deleted successfully' }), {
         status: 200,
         headers: {
            'Content-Type': 'application/json',
         },
